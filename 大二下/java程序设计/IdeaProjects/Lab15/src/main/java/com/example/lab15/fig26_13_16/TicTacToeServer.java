package com.example.lab15.fig26_13_16;
...
public class TicTacToeServer extends JFrame 
{
   ...
   public boolean validateAndMove( int location, int player )
   {
      ...
      if ( !isOccupied( location ) &&!isGameOver())
      {
   ...
   public boolean isGameOver()
   {
      ...
      for(int i=0;i<2;i++){
         if(board[i*4].equals(board[(i+1)*4])&&!board[i*4].equals(""))left_count++;
         if(board[(i+1)*2].equals(board[(i+2)*2])&&!board[(i+1)*2].equals(""))right_count++;
      }
      if(left_count==2)return true;
      if(right_count==2)return true;
      for(int i=0;i<3;i++){
         int count_column = 0,count_row = 0;
         for(int j=0;j<2;j++){
            if(board[3*i+j].equals(board[3*i+j+1])&&!board[3*i+j].equals(""))count_column++;
            if(board[3*j+i].equals(board[3*(j+1)+i])&&!board[3*j+i].equals(""))count_row++;
         }
         if(count_column==2)return true;
         if(count_row==2)return true;
      }
      int count_all = 0;
      for(int i=0;i<9;i++){
         if(!board[i].equals(""))count_all++;
      }
      if(count_all==9)return true;
