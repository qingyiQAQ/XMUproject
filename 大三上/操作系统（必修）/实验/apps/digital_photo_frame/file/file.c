...
int MapFile(PT_FileMap ptFileMap)
{
	...
	if (tFp == NULL)
	{
		DBG_PRINTF("can't open %s: %s\n", ptFileMap->strFileName, strerror(errno));
...
static int isRegFile(char *strFilePath, char *strFileName)
{
	...
	iFd = open(strTmp, O_RDONLY);
	if (iFd < 0) {
		//printf("%s %s %d, can not open %s\n", __FILE__, __FUNCTION__, __LINE__, strTmp);
		return 0;
	}
	else
    {
...
int GetDirContents(char *strDirName, PT_DirContent **pptDirContents, int *piNumber)	
{
	...
	//printf("%s %s %d, strDirName = %s\n", __FILE__, __FUNCTION__, __LINE__, strDirName);
	EatMoreSplash(strDirName);
	
	/* 扫描目录,结果按名字排序,存在aptNameList[0],aptNameList[1],... */
	iNumber = scandir(strDirName, &aptNameList, 0, alphasort);
	//printf("%s %s %d, iNumber = %d\n", __FILE__, __FUNCTION__, __LINE__, iNumber);
	if (iNumber < 0)
	{
	...
	aptDirContents = malloc(sizeof(PT_DirContent) * iNumber);
    ...
    *pptDirContents = aptDirContents;
	...
	for (i = 0, j = 0; i < iNumber; i++)
	{
		...
		//printf("%s %s %d, %s is dir ?\n", __FILE__, __FUNCTION__, __LINE__, aptNameList[i]->d_name);
