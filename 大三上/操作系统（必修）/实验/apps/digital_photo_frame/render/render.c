...
int GetPixelDatasForIcon(char *strFileName, PT_PixelDatas ptPixelDatas)
{
	...
	if (strstr(strFileName, "bmp"))
		pfileParser = Parser("bmp");
	else
		pfileParser = Parser("jpg");

	iError = pfileParser->isSupport(&tFileMap);
    ...
    iError = pfileParser->GetPixelDatas(&tFileMap, ptPixelDatas);
    if (iError)
    {
...
int isPictureFileSupported(char *strFileName)
{
	...
	EatMoreSplash(tFileMap.strFileName);
    iError = MapFile(&tFileMap);
    if (iError)
    {
        DBG_PRINTF("MapFile %s error!\n", strFileName);
