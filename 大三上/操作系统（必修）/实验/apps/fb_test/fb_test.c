...
int main(int argc, char** argv)
{
	...
	if ((int)fb_base_addr == -1) {
		...
		for (j = 0; j < fb_var.yres; j++)
			write(fd, tmp_buf, fb_var.xres * fb_var.bits_per_pixel / 8);
    ...
    {
    	/*blank	*/
		printf("blank frame buffer\n");
    	memset(fb_base_addr,0x00,screensize);

    	//usleep(50000);
		printf("mire frame buffer\n");
    	mire();
    }
