...
def getFeeders(graph, Line_lst):
    ...
    for line in Line_lst:
        ...
        Feeders = set(list(Feeders["name"].values))
        Feeder_set = Feeder_set | Feeders
...
def getLineRels(graph, Line_lst, topk):
    ...
    for line in Line_lst:
        ...
        Line_rel_set += Line_rel_lst

    Line_rel_set = list(set(Line_rel_set) - set(Line_lst))
    return Line_rel_set
...
def queryData(graph, Line_lst, topk=3):
    ...
    # get Feeder nodes
    Feeder_set = getFeeders(graph, Line_lst)

    # get CEMs with time (time from Meta node)
    CEM_lst = getCEMs(graph, Feeder_set)

    # get CEMs split by time (order: full, 2020, 2021, 2022)
    CEM_all = getCEMsbyTime(CEM_lst)

    ## Case2: Use TCEKG
    # get 1-hop Line nodes
    Line_hop_set = getLineRels(graph, Line_lst, topk)

    # get 1-hop Feeder nodes
    Feeder_hop_set = getFeeders(graph, Line_hop_set)

    # get 1-hop CEMs with time (time from Meta node)
    CEM_hop_lst = getCEMs(graph, Feeder_hop_set)

    # get related CEMs split by time (order: full, 2020, 2021, 2022)
    CEM_hop_all = getCEMsbyTime(CEM_hop_lst)

    return CEM_all, CEM_hop_all